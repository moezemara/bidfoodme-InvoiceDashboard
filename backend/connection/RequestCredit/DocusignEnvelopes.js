...
export default class DocusignEnvelopes {
    ...
    SelectEnvelopeByEnvelopeId(data) {
        return new Promise((resolve, reject) =>{
            this.pool.query(
                `SELECT * FROM requestcredit_docusign_envelopes WHERE envelope_id = ?`,
                [
                    data.envelope_id
                ],
    ...
    UpdateEnvelopeStatusAndSigningDate(data) {
        return new Promise((resolve, reject) =>{
            this.pool.query(
                `UPDATE requestcredit_docusign_envelopes SET status = ?, signing_date = ? WHERE envelope_id = ?`,
                [
                    data.status,
                    data.signing_date,
                    data.envelope_id
    ...
    InsertEnvelope(data) {
        return new Promise((resolve, reject) =>{
            this.pool.query(
                `INSERT INTO requestcredit_docusign_envelopes (envelope_id, application_id, recipient_email) VALUES (?, ?, ?)`,
                [
                    data.envelope_id,
                    data.application_id,
