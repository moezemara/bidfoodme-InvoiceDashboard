...
export default class BankInfo {
    ...
    SelectBankInfoByApplicationId(data) {
        return new Promise((resolve, reject) =>{
            this.pool.query(
                `SELECT * FROM bank_info WHERE application_id = ?`,
                [
                    data.application_id
                ],
                (error, results, fields) => {
    ...
    UpdateBankInfoByApplicationId(data) {
        return new Promise((resolve, reject) =>{
            this.pool.query(
                `UPDATE bank_info SET bank_name = ?, bank_city = ?, bank_account_number = ?, bank_iban = ?, bank_swift = ?, bank_account_type = ? WHERE application_id = ?`,
                [
                    data.bank_name,
                    data.bank_city,
    ...
    InsertBankInfo(data) {
        return new Promise((resolve, reject) =>{
            this.pool.query(
                `INSERT INTO bank_info (application_id, bank_name, bank_city, bank_account_number, bank_iban, bank_swift, bank_account_type) VALUES (?, ?, ?, ?, ?, ?, ?)`,
                [
                    data.application_id,
                    data.bank_name,
                    data.bank_city,
