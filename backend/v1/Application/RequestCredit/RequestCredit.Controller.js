...
export async function LoadSavedProgress(req, res) {
    ...
    try {
        ...
        const get_authorised_signatures_info_action = await database.RequestCredit.AuthorisedSignatures.SelectAuthorisedSignaturesByApplicationId({application_id: active_application.application_id})
    ...
    try {
        ...
        switch (step) {
            ...
            case 'signatures':
                ...
                const delete_signatures_info_action = await database.RequestCredit.AuthorisedSignatures.DeleteSignaturesByApplicationId({application_id: application_id})

                // loop on signatures and add them one by one
                for (let i = 0; i < data.length; i++) {
                    const signature = data[i];
                    const create_signatures_info_action = await database.RequestCredit.AuthorisedSignatures.InsertAuthorisedSignatures({application_id: application_id, ...signature})
                }
                break;
            case 'requests':
